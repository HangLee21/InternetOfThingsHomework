...
class Player:
    ...
    def __init__(self):
        ...
        for a in range(self.JUMP_FREQ_AREA_PER_SYMBOL * self.SYMBOL_PER_DURATION):
            self.F_MAP.extend(
                [self.BASE_FREQ + self.AREA_GAP * a + i * (self.FGAP * self.GAP_RATE) for i in range(self.CHANNELS)])
        print(self.F_MAP)
    ...
    def compute_target_frequency(self, chord, jump_index, symbol_index):
        return self.F_MAP[
            symbol_index * self.CHANNELS * self.JUMP_FREQ_AREA_PER_SYMBOL + jump_index * self.CHANNELS + chord]
    ...
    def decode_once(self, signal, data_queue=None):
                ...
                for i in range(offset, len(signal) - step, step):
                    ...
                    for j in range(self.SYMBOL_PER_DURATION):
                        possible_bit = self.select_frequency(fft_result, fft_freqs, power_threshold,
                                                             jump_index=jump_index, symbol_index=j)
        ...
        for i in range(offset, len(signal) - step, step):
            ...
            fft_freqs = np.fft.fftfreq(len(fft_result), 1 / self.F_SAMPLE_RATE)
            for j in range(self.SYMBOL_PER_DURATION):
                possible_bit = self.select_frequency(fft_result, fft_freqs, power_threshold, jump_index=jump_index,
                                                     symbol_index=j)
                sliced.append(possible_bit)
            jump_index = (jump_index + 1) % self.JUMP_FREQ_AREA_PER_SYMBOL

        msg = sliced[:self.MSG_SIZE * self.SYMBOL_PER_DURATION]
        result = msg[len(self.PREAMBLE_CODE) * self.SYMBOL_PER_DURATION: (
                                                                                     len(self.PREAMBLE_CODE) + self.PAYLOAD_SIZE) * self.SYMBOL_PER_DURATION]
    ...
    def keep_recording(self, record_length=100, data_queue=None):
        ...
        with wave.open(fn, 'wb') as wf:
            ...
            stream = p.open(format=pyaudio.paFloat32, channels=1, rate=self.F_SAMPLE_RATE, input=True,
