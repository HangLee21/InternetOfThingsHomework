...
def envelope_detection(signal, sample_rate):
    ...
    # 对整流后的信号应用低通滤波器
    envelope = bandpass_filter(rectified_signal, 1, 10, sample_rate)  # 低通滤波，平滑包络
    return envelope
...
def demodulate_fsk(signal, f1, f2, sample_rate, bit_duration):
    ...
    signal = bandpass_filter(signal, f1 - 500, f2 + 500, sample_rate)  # 带通滤波
    # 计算信号的比特数
...
# 示例：执行解码流程
use_file_input = True  # 设置为 False 来实时接收音频信号
if use_file_input:
    directory = 'output/'
